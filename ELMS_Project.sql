...
# got error phone out of range, so changed from int to varchar(15)
...
drop trigger if exists leave_requests_BEFORE_UPDATE;

# -- after update
#if old.status != new.status then
insert into leave_history(leave_id,emp_id,old_status,new_status) values(new.leave_id,new.emp_id,old.status,new.status);
#enf if;
...
insert into leave_request_log(leave_id,emp_id,leave_type_id) values(new.leave_id,new.emp_id,new.leave_type_id);
